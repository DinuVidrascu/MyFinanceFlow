    ...
    <div className="space-y-5 tab-animate">
      ...
      <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 mb-4">
        ...
        <p className="text-xs text-indigo-600">
          Cost total estimat (articole neselectate): <span className="font-bold text-lg block mt-1">{formatCurrency(notesTotalImpact)}</span>
        </p>
      </div>
      <div className="space-y-4">
        {noteGroups.map(group => {
          const groupTotal = getGroupTotal(group.items);
          const uncheckedCount = group.items.filter(i => !i.checked).length;
              ...
              <div className="p-4 bg-gray-50 flex justify-between items-center border-b border-gray-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400">{group.items.length} articole ({uncheckedCount} active)</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <div className="text-right mr-2">
                    <span className="block text-[10px] text-gray-400 uppercase">Total</span>
                    <span className="font-bold text-indigo-600">{formatCurrency(groupTotal)}</span>
                  </div>
                  <button onClick={() => openNoteGroupModal(group)} className="p-2 text-gray-400 hover:text-indigo-600 hover:bg-white rounded-lg transition">
                    <Edit2 size={18} />
                  </button>
                  <button onClick={() => handleDeleteGroup(group.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-white rounded-lg transition">
                    <Trash2 size={18} />
                  </button>
                </div>
              </div>
                ...
                {group.items.length === 0 ? (
                  ...
                  <div className="space-y-1">
                    {group.items.map(item => (
                      <div
                        key={item.id}
                        onClick={() => toggleSubItemCheck(group.id, item.id)}
                      ...
                      >
                        <div className="flex items-center gap-2 overflow-hidden">
                          {item.checked ? <CheckCircle size={16} className="text-green-500 flex-shrink-0" /> : <Circle size={16} className="text-gray-300 flex-shrink-0" />}
                          <span className={`text-sm truncate ${item.checked ? 'line-through text-gray-400' : 'text-gray-700'}`}>{item.text}</span>
                        </div>
                        <span className="text-xs font-semibold text-gray-500 flex-shrink-0">{formatCurrency(item.cost)}</span>
                      </div>
                    ))}
                  </div>
                )}
